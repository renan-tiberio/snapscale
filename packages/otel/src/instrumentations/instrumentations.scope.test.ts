...
const getNodeAutoInstrumentationsSpy = vi.fn((config: AutoInstrumentationsConfig): unknown[] => {
  void config
  return []
})

const lastConfig = (): AutoInstrumentationsConfig => {
  const call = getNodeAutoInstrumentationsSpy.mock.calls.at(-1)
  if (!call) {
