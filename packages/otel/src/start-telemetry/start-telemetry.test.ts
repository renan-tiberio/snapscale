...
// A spy standing in for the real Node SDK class: if OTEL_ENABLED is off,
// this constructor — and therefore `.start()` — must never run.
...
describe('startTelemetry', () => {
  ...
  it('returns a handle exposing shutdown()', async () => {
    const handle = await startTelemetry({ serviceName: 'snapscale-test', env: {} })

    expect(typeof handle.shutdown).toBe('function')
  })

  it('never constructs or starts the Node SDK when OTEL_ENABLED is unset', async () => {
    await startTelemetry({ serviceName: 'snapscale-test', env: {} })
  ...
  it('never constructs or starts the Node SDK when OTEL_ENABLED=false', async () => {
    await startTelemetry({ serviceName: 'snapscale-test', env: { OTEL_ENABLED: 'false' } })
  ...
  it('starts the Node SDK when OTEL_ENABLED=true', async () => {
    await startTelemetry({ serviceName: 'snapscale-test', env: { OTEL_ENABLED: 'true' } })
  ...
  it('crashes naming the field for a bad OTEL_EXPORTER value, enabled or not', async () => {
    await expect(
      startTelemetry({ serviceName: 'snapscale-test', env: { OTEL_EXPORTER: 'bogus' } }),
