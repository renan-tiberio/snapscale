...
describe('otpRepo', () => {
  ...
  it('stores a code with zero attempts and no consumed_at', async () => {
    const code = await otpRepo.create(database.db, {
      email: EMAIL,
      codeHash: 'hash-1',
      salt: 'salt-1',
  ...
  it('findActiveByEmail returns the unexpired unconsumed code and ignores expired ones', async () => {
    await otpRepo.create(database.db, {
      email: EMAIL,
      codeHash: 'expired',
      salt: 'salt',
      expiresAt: oneMinuteAgo(),
    })
    const active = await otpRepo.create(database.db, {
      email: EMAIL,
      codeHash: 'active',
      salt: 'salt',
      expiresAt: inTenMinutes(),
    ...
    expect(await otpRepo.findActiveByEmail(database.db, EMAIL)).toMatchObject({ id: active.id })
  })
  ...
  it('findActiveByEmail returns undefined once the code is consumed', async () => {
    ...
    const consumed = await otpRepo.markConsumed(database.db, code.id)
  ...
  it('incrementAttempts raises the counter by one per call', async () => {
    ...
    expect((await otpRepo.incrementAttempts(database.db, code.id))?.attempts).toBe(1)
    expect((await otpRepo.incrementAttempts(database.db, code.id))?.attempts).toBe(2)
    expect((await otpRepo.findActiveByEmail(database.db, EMAIL))?.attempts).toBe(2)
  })

  it('incrementAttempts returns undefined for an unknown id', async () => {
    expect(await otpRepo.incrementAttempts(database.db, randomUUID())).toBeUndefined()
  })

  it('markConsumed returns undefined for an unknown id', async () => {
    expect(await otpRepo.markConsumed(database.db, randomUUID())).toBeUndefined()
  })
  ...
  it('invalidateActiveForEmail consumes every active code of that email only', async () => {
    ...
    const invalidated = await otpRepo.invalidateActiveForEmail(database.db, EMAIL)

    expect(invalidated).toBe(2)
    expect(await otpRepo.findActiveByEmail(database.db, EMAIL)).toBeUndefined()
    expect(await otpRepo.findActiveByEmail(database.db, OTHER_EMAIL)).toMatchObject({
      codeHash: 'other',
    })
  })

  it('invalidateActiveForEmail reports zero when nothing is active', async () => {
    expect(await otpRepo.invalidateActiveForEmail(database.db, EMAIL)).toBe(0)
  })
})
