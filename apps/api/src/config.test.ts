...
describe('loadConfig', () => {
  ...
  it('respects an explicit OTP_TTL_SECONDS override instead of the default', () => {
    const config = loadConfig({ ...validEnv, OTP_TTL_SECONDS: '120' })
  ...
  it('crashes naming the field when JWT_SECRET is missing', () => {
    ...
    expect(() => loadConfig(envWithoutSecret)).toThrowError(/JWT_SECRET/)
  })
  ...
  it('crashes naming the field when DATABASE_URL is missing', () => {
    ...
    expect(() => loadConfig(envWithoutDb)).toThrowError(/DATABASE_URL/)
  })

  it('crashes naming the field when SMTP_PORT is not a number', () => {
    expect(() => loadConfig({ ...validEnv, SMTP_PORT: 'not-a-port' })).toThrowError(/SMTP_PORT/)
  })
