...
const createCapturingLogStream = (): { stream: Writable; lines: () => string[] } => {
...
describe('GET /auth/file-token (scoped file tokens)', () => {
  ...
  beforeEach(async () => {
    ...
    sessionToken = await app.jwt.sign({
      sub: ownerId,
  ...
  const uploadImage = async (): Promise<ImageBody> => {
  ...
  it('responds 200 with a `scope: "file"`, 60s-lived token for a valid session', async () => {
    ...
    const payload = app.jwt.verify(body?.token ?? '') as {
      sub: string
      scope: string
      iat: number
      exp: number
