...
/**
 * `GET /auth/me` (docs/03 §4) — the endpoint the web's AuthProvider needs to
 * validate a `localStorage` token against the server instead of trusting it.
 * A token is only a claim: if the account behind it is gone, the answer has
 * to be a 404, not a user assembled from the token's own payload.
 */
...
function uniqueEmail(): string {
  return `me-${randomUUID()}@example.com`
}
...
describe('GET /auth/me', () => {
  ...
  beforeEach(async () => {
    ...
    email = uniqueEmail()
    userId = (await usersRepo.upsertByEmail(database.db, email)).id
    token = await app.jwt.sign({ sub: userId, email, scope: 'session' })
  })
  ...
  it('reads the user from the database, not from the token payload', async () => {
    // A token whose `email` claim disagrees with the stored row: the answer
    // must be the row, otherwise the route validates nothing.
    const staleToken = await app.jwt.sign({ sub: userId, email: 'stale@example.com', scope: 'session' })
    ...
    expect(response.statusCode).toBe(200)
    expect((response.json() as Envelope<MeBody>).data?.user.email).toBe(email)
  })
