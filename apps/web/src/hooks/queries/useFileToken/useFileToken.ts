 ...
 * going silent.
 */
const FILE_TOKEN_FALLBACK_REFETCH_MS = 30_000 // 30 seconds
/** Floor so a token that is already dead (or nearly so) still gets retried, not hammered. */
const FILE_TOKEN_MIN_REFETCH_MS = 1_000 // 1 second
/**
...
/** Refetch at half the remaining life of the token, not all of it — see `msUntilNextRefresh`. */
const REMAINING_LIFE_REFRESH_FRACTION = 2

export type UseFileTokenResult = {
...
type MsUntilNextRefreshParams = { token: string | undefined }
 ...
 */
const msUntilNextRefresh = ({ token }: MsUntilNextRefreshParams): number => {
  if (token === undefined) {
  ...
  return Math.max(remainingMs / REMAINING_LIFE_REFRESH_FRACTION, FILE_TOKEN_MIN_REFETCH_MS)
}
 ...
 */
export const useFileToken = (): UseFileTokenResult => {
  ...
  const query = useQuery<FileTokenResponse, ApiError>({
    ...
    refetchInterval: (currentQuery) =>
      msUntilNextRefresh({ token: currentQuery.state.data?.token }),
  ...
  const fileToken = rawToken !== undefined && isJwtLive({ token: rawToken }) ? rawToken : null
