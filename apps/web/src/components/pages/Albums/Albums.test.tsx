...
describe('Albums', () => {
  ...
  describe('with the seeded album list', () => {
    beforeEach(() => {
    ...
    it('signs the user out back to the login screen', async () => {
      ...
      expect(
        await screen.findByRole('heading', { name: 'Sign in to SnapScale' }),
      ).toBeInTheDocument()
    })
  })

  it('invites the user to create the first album when there is none', async () => {
    server.use(http.get(`${API_BASE}/albums`, () => HttpResponse.json(ok({ data: [] }))))
